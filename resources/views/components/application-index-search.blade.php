...
$action = $action ?? route('applications.index', $job);
...
$hasFilters        = request()->hasAny(['q', 'status', 'date_from', 'date_to']);
$activeFilterCount = collect(['status', 'date_from', 'date_to'])
...
<form method="GET" action="{{ $action }}"
    ...
    <div x-show="filtersOpen"
        ...
        <div class="grid gap-4 sm:grid-cols-3">
            ...
            {{-- Date from --}}
            <div>
                <x-input-label :value="__('Applied from')" />
                <x-text-input
            ...
            {{-- Date to --}}
            <div>
                <x-input-label :value="__('Applied to')" />
                <x-text-input
